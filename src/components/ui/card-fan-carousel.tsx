...
function getHeightMultiplier(width: number) {
  ...
  let idealPx: number;
  if (width < 480) idealPx = 18 * 16;       // 288px
  else if (width < 640) idealPx = 22 * 16;  // 352px
  else if (width < 768) idealPx = 25 * 16;  // 400px
...
export default function SocialCards({ cards, onCardClick }: SocialCardsProps) {
  ...
  useEffect(() => {
    ...
    const cardElements: HTMLElement[] = Array.from(container.querySelectorAll(".fan-card"));
    ...
    cardElements.forEach((card: HTMLElement, cardIndex: number) => {
    ...
    cardElements.forEach((el: HTMLElement, i: number) => {
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20 overflow-visible">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-full h-full overflow-hidden rounded-2xl md:rounded-3xl border border-[#C5A059]/30 shadow-[0_12px_32px_rgba(42,36,33,0.12)]">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
                <div className="absolute inset-0 bg-gradient-to-t from-brand-charcoal/70 via-transparent to-transparent z-20 opacity-60 hover:opacity-40 transition-opacity" />
                {card.alt && (
                  <div className="absolute inset-x-0 bottom-0 p-3 md:p-4 z-30 text-center">
                    <span className="text-[11px] md:text-xs text-brand-cream font-serif tracking-wider drop-shadow-sm line-clamp-1 block">
                      {card.alt}
                    </span>
                  </div>
                )}
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer" onClick={handleClick}>{image}</a>
            ) : (
              <div key={index} className="fan-card cursor-pointer" onClick={handleClick}>{image}</div>
            );
          })}
        </div>
      </div>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-9 sm:mt-6 md:mt-8 z-30">
          <button className={`${ARROW_CLASSES} w-10 h-10 md:w-12 md:h-12`} onClick={() => cycle("left")} aria-label="Previous">
            {chevron("left")}
          </button>
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-[#C5A059] scale-[1.4]" : "bg-[#C5A059]/30"}`} />
            ))}
          </div>
