...
// Helper to determine page ID from URL path
function getPageFromPath(path: string): string {
  ...
  if (clean === 'booking' || clean === 'book') return 'booking';
  return 'home';
}
...
export default function App() {
  ...
  // Sync state with URL pathname & browser history
  ...
  // Handle service bookings from cards
    ...
    <>
      ...
      <AnimatePresence mode="wait">
        {loading && (
          ...
          >
            <div className="text-center space-y-8 px-6 max-w-md relative flex flex-col items-center">
              ...
              <div className="space-y-4 w-64">
                ...
                <div className="flex flex-col items-center space-y-1">
                  <span className="text-[10px] text-brand-gold-light/85 tracking-widest font-serif italic uppercase h-4 block transition-all duration-300">
                    {loadingProgress < 35 && "Calibrating sanctuary active concentrates..."}
                    {loadingProgress >= 35 && loadingProgress < 75 && "Preparing cellular skin therapies..."}
                    {loadingProgress >= 75 && "Welcome to Highlights..."}
                  </span>
      ...
      {!loading && (
        ...
        >
          ...
          <main id="main-content" className="flex-1 pt-20">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {currentPage === 'offers' && (
                  <Offers onReserveClick={handleReserveClick} />
                )}
                {currentPage === 'reviews' && (
                  <Reviews onReserveClick={handleReserveClick} />
                )}
          ...
          {/* Slide-Over Drawer for "My Bookings History" */}
          <AnimatePresence>
            {showBookingsTray && (
              <motion.div
                id="tray-backdrop"
                ...
                className="fixed inset-0 z-50 bg-brand-charcoal/50 backdrop-blur-xs flex justify-end"
              >
                <motion.div
                  id="tray-panel"
                  initial={{ x: '100%' }}
                  ...
                  <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-1">
                    {localBookings.length > 0 ? (
                      localBookings.map((b) => (
                        ...
                        >
                          <button
                            ...
                            className="absolute top-4 right-4 text-xs font-semibold text-stone-400 hover:text-rose-500 cursor-pointer"
                          >
                            Cancel
                          </button>
                      ...
                      <div className="text-center py-20 space-y-4">
                        <Heart className="w-12 h-12 text-brand-rose/40 mx-auto" />
                        <div>
                          <h4 className="font-serif text-lg font-medium text-brand-charcoal">No Active Bookings</h4>
                          <p className="text-xs text-stone-400 mt-1 max-w-xs mx-auto">
                            You have no upcoming appointments in this browser session.
                          </p>
                        </div>
                        <button
